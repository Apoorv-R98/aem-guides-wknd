 ...
 ******************************************************************************/
// WKND OVERLAY (GRANITE-71500): aem-core-wcm-components main's search.js with PR #3069's
// (GRANITE-71181, branch fix/GRANITE-71181-search-v3-fixes) null-guard diff applied - that PR is
// still open, so this is a manually-ported diff, not a verbatim copy of a merged commit.
// Delete this file and its clientlib folder once WKND bumps to a release that includes #3069 -
// see docs/superpowers/specs/2026-08-05-wknd-contentai-semantic-search-demo-design.md §5.2.
(function() {
    "use strict";
    ...
    var AI_SEARCH_PREFIX = "?{}?"; // prefix routing the query to ContentAI-powered semantic search
    ...
    function Search(config) {
        ...
        if (this._elements.aiToggle) {
            this._aiSearchEnabled = this._elements.aiToggle.checked;
            this._elements.aiToggle.addEventListener("change", this._onAiToggleChange.bind(this));
        }
    ...
    Search.prototype._onAiToggleChange = function(event) {
        this._aiSearchEnabled = this._elements.aiToggle.checked;
        this._onInput();
    };
    ...
    Search.prototype._updateResults = function() {
        ...
        if (self._hasMoreResults) {
            ...
            var queryString = serialize(self._elements.form);
            if (self._aiSearchEnabled) {
                queryString = queryString.replace("fulltext=", "fulltext=" + encodeURIComponent(AI_SEARCH_PREFIX));
            }
            var url = self._action + "?" + queryString + "&" + PARAM_RESULTS_OFFSET + "=" + self._resultsOffset;
    ...
    // Guarded: this v3-only element may be absent if this shared JS ends up initializing
    // against older v1 markup on the same page.
